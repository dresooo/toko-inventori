...
document.addEventListener("DOMContentLoaded", () => {
    ...
    confirmCrop.addEventListener("click", () => {
        ...
        // Tambahkan hasil crop ke Fabric.js canvas
        fabric.Image.fromURL(croppedDataURL, (img) => {
            // Auto fit ke ukuran canvas
            const canvasWidth = pinCanvas.width;
            const canvasHeight = pinCanvas.height;

            // Hitung rasio skala agar gambar muat sepenuhnya di canvas
            const scale = Math.min(
                canvasWidth / img.width,
                canvasHeight / img.height
            );

            // Terapkan skala dan posisi ke tengah
            img.scale(scale);
            img.set({
                left: canvasWidth / 2,
                top: canvasHeight / 2,
